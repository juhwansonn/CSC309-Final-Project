...
const ManageEventPage = () => {
  ...
  const addOrganizer = async () => {
    try {
      await axios.post(
        `${API_BASE_URL}/events/${id}/organizers`,
  ...
  const awardPoints = async (targetUtorid = null) => {
    ...
    const confirmMsg = targetUtorid
      ...
      : `Award ${amount} points to ALL ${event.guests.length} guests? (Total: ${
        ...
        <div>
          ...
          <ul style={{ listStyle: "none", padding: 0 }}>
            {event.organizers?.map((org) => (
              ...
              >
                <span>
                  {org.name} <small>@{org.utorid}</small>
                </span>
                <button
        ...
        <div>
          ...
          >
            ...
            <p style={{ fontSize: "0.9rem", color: "#666" }}>
              Points Remaining in Budget: <strong>{event.pointsRemain}</strong>
            </p>
          ...
          >
            {event.guests?.map((guest) => (
              ...
              >
                ...
                <div style={{ display: "flex", gap: "5px" }}>
                  <button
                    onClick={() => awardPoints(guest.utorid)}
                    style={{
                      fontSize: "0.8rem",
            ...
            {event.guests?.length === 0 && (
              <li style={{ padding: "10px" }}>No guests yet.</li>
            )}
          </ul>
        </div>
      </div>
