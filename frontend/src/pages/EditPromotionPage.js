...
const EditPromotionPage = () => {
  ...
  const fetchPromotion = useCallback(async () => {
    try {
      ...
      setError(
        err.response?.data?.error || "Failed to load promotion details."
      );
      setLoading(false);
    }
  }, [id, token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.minSpending !== "")
        payload.minSpending = parseFloat(formData.minSpending) || null;
      if (formData.rate !== "")
        payload.rate = parseFloat(formData.rate) || null;
      if (formData.points !== "")
        payload.points = parseInt(formData.points) || null;
    ...
    <div style={{ padding: "20px", maxWidth: "500px" }}>
      ...
      >
        ...
        <input
          ...
          type="number"
          step="0.1"
          placeholder="Multiplier Rate (e.g. 1.5)"
          value={formData.rate}
          onChange={handleChange}
        />
        <input
